...
require_once("./../Uploads/FileUploader.php");

	$sessionEmail = null;
	if (!isset($_SESSION['email'])) { header("location:./views/login/login.php"); }
	else{ $sessionEmail = $_SESSION['email']; }

	if (isset($_POST['upload'])) {		
		$image = "./../Uploads/user_images/".$_FILES['fileToUpload']['name'];

// The code commented below works but i have used external to make this happen
		
		// $target = "./../Uploads/user_images/".basename($_FILES['userImage']['name']);
		// if (isset($image)) {
		// 	$uploadImage = "UPDATE admin SET user_image = '$image' WHERE Email = '$sessionEmail'";
		// 	mysqli_query($connection, $uploadImage);
		// 	if (move_uploaded_file($_FILES['userImage']['tmp_name'], $target)) {
		// 		echo '<div id="uploadMsg" style="color:green;">Image Uploaded successfully</div>';
		// 	}else{
		...
		$crud=new FileUploader();
		if (isset($image)) {
			// $uploadImage = "UPDATE admin SET user_image = '$image' WHERE Email = '$sessionEmail'";
			// mysqli_query($connection, $uploadImage);
			$crud->UploadThisFile("user", $image, $sessionEmail);
		}else{ echo "No image";}
...
<form enctype="multipart/form-data" id="changeProfilePicture" action="<?php echo htmlentities($_SERVER['PHP_SELF']);?>" method="post">
  ...
  <div id="uploadUserImg" title="Click to change your profile picture"><input name="fileToUpload" type="file"  onclick="showPostBtn()" /></div>
		...
		<div class="dashBoardContent" id="dashBoardContent">
			<div class="topContainer">	
				<div class="dashContainer">
					<div class="firstRow">
						<div class="item" onclick="makeChanges(6)">
							<h4>Notification</h4><hr>
							<img src="./../Images/icons/notification.png">
						</div>
						<div class="item"  onclick="makeChanges(1)">
							<h4>Profile</h4><hr>
							<img src="./../Images/icons/profile.png">
						</div>
						<div class="item" onclick="makeChanges(2)">
							<h4>Add New Election</h4><hr>
							<img src="./../Images/icons/add.png">
						</div>
						<div class="item" onclick="makeChanges(3)">
							<h4>Add New Candidate</h4><hr>
							<img src="./../Images/icons/addUser.png">
						</div>
					</div>					
					<div class="secondRow">
						<div class="item" onclick="makeChanges(4)">
							<h4>Add Voter</h4><hr>
							<img src="./../Images/icons/addUser.png">
						</div>
						<div class="item" onclick="makeChanges(5)">
							<h4>Show Election Results</h4><hr>
							<img src="./../Images/icons/result.png">
						</div>
					</div>
				</div>
